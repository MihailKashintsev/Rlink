...
def run_echo_forever(config_path: Path) -> None:
    ...
    relay = (cfg.get("relay_url") or DEFAULT_RELAY_WS).strip()
    keys_path = Path(cfg.get("keys_path") or "rlink_bot_keys.json").expanduser()
    if not keys_path.is_absolute():
        keys_path = config_path.expanduser().resolve().parent / keys_path
    keys = BotKeys.from_json_dict(json.loads(keys_path.read_text(encoding="utf-8")))
    sess = RelayBotSession(relay, keys)
    try:
        sess.connect(nick="@" + str(cfg.get("handle") or "bot")[:32])

        def on_dm(sender: str, text: str) -> None:
            sess.send_dm(sender, f"echo: {text[:240]}")

        sess.recv_loop(on_dm, log=print)
    finally:
        sess.close()
