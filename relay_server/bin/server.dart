...
void _handleAdminChannelVerify(_User user, Map<String, dynamic> msg) {
  ...
  final row = _channelDirectory[channelId] ??= <String, dynamic>{
    'channelId': channelId,
    'isPublic': true
  };
...
/// Sends one Web Push to [sub]. [payload] is encrypted for the subscription
...
Future<int> _sendWebPush(
  ...
  if (payload != null && keys is Map) {
    try {
      ...
      stdout.writeln(
          '[RLINK][Push] encrypt failed (${Uri.parse(endpoint).host}): $e');
    }
  }
  if (body != null) {
...
Future<void> _notifyRecipientQueued({
  ...
  final senderNick = _users[senderKey]?.nick.trim() ?? '';
  ...
  try {
    for (final sub in subs) {
      ...
      try {
        final status = await _sendWebPush(client, sub, payload);
...
void _persistBlockedLists() {
  try {
    final serializable = _blockedByUser.map((k, v) => MapEntry(k, v.toList()));
        ...
        if (user == null) {
          ...
          try {
            ...
            for (final other in _users.values) {
              ...
              if (other.away)
                continue; // don't show backgrounded peers as online
              try {
                ws.sink.add(jsonEncode({
                  'type': 'presence',
...
Future<shelf.Response> _infoHandler(shelf.Request request) async {
  ...
  if (request.url.path == 'drive-audio') {
    ...
    try {
      ...
      return _jsonResponse(
          {'ok': false, 'error': 'proxy_failed', 'detail': '$e'},
          status: 502);
    }
  }
  ...
  if (request.url.path == 'drive-proxy') {
    ...
    try {
      ...
      final ct =
          resp.headers.contentType?.mimeType ?? 'application/octet-stream';
      final headers = <String, String>{
        'content-type': ct,
        'access-control-allow-origin': '*',
        'cache-control': 'public, max-age=3600',
  ...
  if (request.url.path == 'push/test' && request.method == 'POST') {
    ...
    try {
      ...
      final decoded = jsonDecode(raw);
      final publicKey =
          (decoded is Map ? (decoded['publicKey'] as String?) : null)
                  ?.trim()
                  .toLowerCase() ??
              '';
      ...
      try {
        for (final sub in subs) {
          ...
          if (endpoint.isEmpty) continue;
          try {
            final status = await _sendWebPush(
                client,
                sub,
