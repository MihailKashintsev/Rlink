...
itself carries no credentials since this repo mirrors a public one.

The app must be built with the *same* TURN_USER/TURN_PASSWORD as
`--dart-define`s (see `.github/workflows/release.yml`) — a mismatch here
doesn't error, it just leaves every call with no TURN fallback.
