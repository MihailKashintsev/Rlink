...
bool _isPublicKeyPeerId(String peerId) =>
    RegExp(r'^[0-9a-fA-F]{64}$').hasMatch(peerId.trim());
...
void _bindIncomingCallOverlay() {
  ...
  void tryShow() {
    ...
    // UI the instant the tab regains any rendering. isInBackground's own
    // listener below re-runs this check when the tab comes back to front, so
    // a call that arrived while backgrounded still gets shown once the user
    // actually returns instead of being silently skipped forever.
...
Future<void> flushOutbox() async {
  ...
  try {
    ...
    for (final m in pending) {
      ...
      if (!_isPublicKeyPeerId(m.peerId)) continue;
      ...
      try {
        if (RelayService.instance.isConnected &&
            RelayService.instance.isPeerKnownOffline(m.peerId)) {
          continue;
        }
  ...
  try {
    ...
    GossipRouter.instance.init(
      ...
      onAck: (fromId, messageId) async {
        ...
        await MediaUploadQueue.instance.markDelivered(messageId);
      ...
      onReact: (fromId, messageId, emoji) async {
        ...
        await ChatStorageService.instance.toggleReaction(messageId, em, fromId);
      },
      ...
      onImgChunkReceived:
        ...
        final isChannelPost =
            ImageService.instance.isChannelPostAssembly(msgId);
      ...
      onProfile: (bleId, publicKey, nick, username, color, emoji, x25519Key,
        ...
        try {
          ...
          // Ищем стаб-контакт для переноса истории:
          // Кейс 1: сообщение пришло раньше профиля → стаб (key=X, nick="X1234...").
          // Кейс 2: контакт создан под BLE UUID до обмена профилями (см. ниже).
          //
          // НЕ матчим по равенству ников (c.nickname == nick) — ник это
          // свободный текст, у двух разных людей он может случайно (или
          // намеренно) совпасть, а этот блок переносит ВСЮ историю на новый
          // publicKey и удаляет старый контакт. Совпадение ников означало
          // «слияние» чужой переписки под ключ постороннего человека — см.
          // security review, 2026-09-23. Настоящая ротация ключа (переустановка
          // у уже известного контакта) требует отдельного, подтверждённого
          // пользователем потока, а не эвристики по свободному тексту.
          Contact? oldContact;
          final allContactsForDedup =
              await ChatStorageService.instance.getContacts();

          // Стаб-контакт: его "ник" — это hex-префикс САМОГО ЭТОГО публичного
          // ключа (проставлен нашим же кодом при получении сообщения раньше
          // профиля), не свободный текст — совпадение здесь криптографически
          // осмысленно, а не совпадение по имени.
          ...
          try {
            oldContact = allContactsForDedup.firstWhere(
              (c) =>
                  ...
                  publicKey
                      .startsWith(c.nickname.replaceAll('...', '')),
            );
          } catch (_) {
            oldContact = null;
          }
    ...
    GossipRouter.instance.onReactionExt = (payload) async {
      ...
      switch (kind) {
        case 'story':
          ...
          break;
        case 'channel_post':
          await ChannelService.instance.togglePostReaction(targetId, em, from);
...
void _bindGossipFallbackHandlersIfMissing() {
  ...
  GossipRouter.instance.onPairRequest ??= (bleId,
      publicKey,
      nick,
      username,
      color,
  ...
  GossipRouter.instance.onProfileReceived ??= (bleId,
      publicKey,
      nick,
      username,
      color,
    ...
    final myProfile = ProfileService.instance.profile;
    if (myProfile != null) {
      unawaited(GossipRouter.instance.broadcastProfile(
        id: myProfile.publicKeyHex,
        nick: myProfile.nickname,
        username: myProfile.username,
        color: myProfile.avatarColor,
          ...
          child: Stack(
            ...
            children: [
              ...
              ValueListenableBuilder<double?>(
                ...
                builder: (ctx, top, _) {
                  final y =
                      top ?? AudioQueueMiniPlayerLayout.defaultBarTop(ctx);
                  return Positioned(
                    left: 0,
                    right: 0,
                    top: y,
